...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!user.password || typeof user.password !== 'string') {
      console.error("[LOGIN ERROR] User has no valid password hash:", user.phone);
      return res.status(401).json({ error: "Tài khoản chưa được thiết lập mật khẩu hợp lệ." });
    }
    ...
    try {
      ...
      // If bcrypt fails, it might be due to an invalid hash format
      if (bcryptError.message && bcryptError.message.includes("pattern")) {
         return res.status(401).json({ error: "Định dạng mật khẩu không hợp lệ. Vui lòng liên hệ hỗ trợ." });
      }
      throw bcryptError;
    }
